...
export default function AllStudentsManager(){
  ...
  const [dataForm,setDataForm]=useState({
    name:"",phone:"",course:"",parentName:"",parentPhone:"",reportVisible:true
  });
  ...
  const openData=async(s:Student)=>{
    ...
    try{
      ...
      setDataForm({
        ...
        parentName:s.parentName,parentPhone:s.parentPhone,reportVisible:s.reportVisible
      });
    ...
    try{
      const res=await fetch("/api/admin/students",{
        ...
        body:JSON.stringify({
          ...
          parent1:{name:dataForm.parentName,phone:dataForm.parentPhone}
        })
      });
      const data=await res.json();
      if(!res.ok) throw new Error(data.error||"تعذر حفظ البيانات");
      setModalMessage("تم حفظ بيانات الطالب ✓");
      await load();
  ...
  return <main className="all-students-page-v76" dir="rtl">
    ...
    {modal&&selected?<div className="student-modal-backdrop-v80" onMouseDown={e=>{if(e.target===e.currentTarget)closeModal()}}>
      <section className={`student-edit-modal-v80 ${modal}`}>
        ...
        {modal==="data"?<div className="student-modal-body-v80">
          <div className="modal-form-grid-v80">
            <label><span>اسم الطالب</span><input value={dataForm.name} onChange={e=>setDataForm({...dataForm,name:e.target.value})}/></label>
            <label><span>موبايل الطالب</span><input value={dataForm.phone} onChange={e=>setDataForm({...dataForm,phone:e.target.value})}/></label>
            <label><span>الكورس</span><select value={dataForm.course} onChange={e=>setDataForm({...dataForm,course:e.target.value})}>{courses.map(c=><option key={c}>{c}</option>)}</select></label>
            <label><span>اسم ولي الأمر</span><input value={dataForm.parentName} onChange={e=>setDataForm({...dataForm,parentName:e.target.value})}/></label>
            <label><span>موبايل ولي الأمر</span><input value={dataForm.parentPhone} onChange={e=>setDataForm({...dataForm,parentPhone:e.target.value})}/></label>
            <label className="modal-check-v80"><input type="checkbox" checked={dataForm.reportVisible} onChange={e=>setDataForm({...dataForm,reportVisible:e.target.checked})}/><span>إظهار التقرير لولي الأمر</span></label>
          </div>
          <button className="modal-save-v80" onClick={saveData} disabled={modalBusy}>حفظ البيانات</button>
        </div>:null}
