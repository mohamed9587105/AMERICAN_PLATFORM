...
export async function GET(){
  ...
  return NextResponse.json({mode:"online",students},{headers:noStoreHeaders});
}

export async function POST(req:Request){
  const adminDenied=await requireAdminApi("students.create");
  if(adminDenied) return adminDenied;
  if(!dbConfigured) return NextResponse.json({mode:"demo",saved:false});
  const body=await req.json();
  try{
    const created=await createStudentWithParents(body);
    await adminAudit("student.created","student",created.student?.id,{name:created.student?.name});
...
export async function PATCH(req:Request){
  const adminDenied=await requireAdminApi("students.edit");
  if(adminDenied) return adminDenied;
  if(!dbConfigured) return NextResponse.json({mode:"demo",saved:false});
  const body=await req.json();
  ...
  await adminAudit("student.updated","student",String(body.id),{});
