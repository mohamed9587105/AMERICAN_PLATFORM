...
type Student={
  ...
  parentName:string;parentPhone:string;reportVisible:boolean;createdAt?:string|null;
};

const uid=()=>Math.random().toString(36).slice(2,9);
const asArray=(v:any)=>Array.isArray(v)?v:(v?[v]:[]);

export default function AllStudentsManager({permissions={edit:true,password:true,toggle:true,importStudents:true,create:true,reports:true,finance:true}}:{permissions?:Record<string,boolean>}){
  ...
  const filtered=useMemo(()=>students.filter(s=>{
    ...
    const dateHit=!registrationDate||String(s.createdAt||"").slice(0,10)===registrationDate;
    return hit&&(course==="all"||s.course===course)&&dateHit;
  }),[students,query,course,registrationDate]);

  const toggleTracking=async(s:Student)=>{
    setRowBusy(`track:${s.id}`);
    try{
      const next=!s.reportVisible;
      const res=await fetch("/api/admin/students",{
        method:"PATCH",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({id:s.id,reportVisible:next})
      });
      const data=await res.json();
      if(!res.ok) throw new Error(data.error||"تعذر تغيير حالة المتابعة");
      setStudents(prev=>prev.map(x=>x.id===s.id?{...x,reportVisible:next}:x));
    }catch(err:any){alert(err.message||"تعذر تغيير حالة المتابعة")}
    finally{setRowBusy("")}
  };

  const resetParentPassword=async(s:Student)=>{
    if(!confirm(`إنشاء باسورد جديد لولي أمر ${s.name}؟ الباسورد القديم سيتوقف فورًا.`)) return;
    setRowBusy(`pass:${s.id}`);
    try{
      const res=await fetch("/api/admin/students/reset-parent-password",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({studentId:s.id})
      });
      const data=await res.json();
      if(!res.ok) throw new Error(data.error||"تعذر إنشاء الباسورد");
      setShownPasswords(prev=>({...prev,[s.id]:data.password||""}));
    }catch(err:any){alert(err.message||"تعذر إنشاء الباسورد")}
    finally{setRowBusy("")}
  };

  const copyPassword=async(s:Student)=>{
    const value=shownPasswords[s.id];
  ...
  return <main className="all-students-page-v76" dir="rtl">
    ...
    {permissions.importStudents?<StudentAdminTools
    ...
    <section className="all-students-toolbar-v76">
      ...
      <label className="students-date-filter-v94"><span>تاريخ التسجيل</span><input type="date" value={registrationDate} onChange={e=>setRegistrationDate(e.target.value)}/></label>
      {registrationDate?<button className="clear-date-v94" onClick={()=>setRegistrationDate("")}>كل التواريخ</button>:null}
    ...
    <section className="all-students-table-wrap-v76">
      <table className="all-students-table-v76">
        <thead><tr><th>الطالب</th><th>الكود</th><th>الكورس</th><th>تاريخ التسجيل</th><th>موبايل الطالب</th><th>ولي الأمر</th><th>موبايل ولي الأمر</th>{permissions.password?<th>باسورد ولي الأمر</th>:null}<th>المتابعة</th><th>الإدارة</th></tr></thead>
        <tbody>{filtered.map(s=><tr key={s.id}>
          <td><strong>{s.name}</strong></td><td>{s.code}</td><td>{s.course||"—"}</td><td><span className="student-created-v94">{s.createdAt?new Intl.DateTimeFormat("ar-EG",{year:"numeric",month:"short",day:"numeric"}).format(new Date(s.createdAt)):"—"}</span></td><td>{s.phone||"—"}</td>
          <td>{s.parentName||"—"}</td><td>{s.parentPhone||"—"}</td>
          {permissions.password?<td>
            <div className="parent-password-cell-v107">
              {shownPasswords[s.id]?<>
                <code>{shownPasswords[s.id]}</code>
                <button className="copy-pass-v107" onClick={()=>copyPassword(s)}>نسخ</button>
              </>:<button className="reset-pass-v107" disabled={rowBusy===`pass:${s.id}`} onClick={()=>resetParentPassword(s)}>
                {rowBusy===`pass:${s.id}`?"جاري الإنشاء...":"إنشاء وإظهار باسورد"}
              </button>}
            </div>
          </td>:null}
          <td>
            {permissions.toggle?<button
              className={`tracking-toggle-v107 ${s.reportVisible?"on":"off"}`}
              disabled={rowBusy===`track:${s.id}`}
              onClick={()=>toggleTracking(s)}
              title={s.reportVisible?"إيقاف برنامج المتابعة":"تفعيل برنامج المتابعة"}
            >
              <span className="tracking-dot-v107"/>
              {rowBusy===`track:${s.id}`?"...":s.reportVisible?"مفعّل":"موقوف"}
            </button>:<span className={`report-status-v76 ${s.reportVisible?"visible":"hidden"}`}>{s.reportVisible?"مفعّل":"موقوف"}</span>}
          </td>
          <td><div className="student-management-actions-v76">
            {permissions.edit?<button className="edit-data" onClick={()=>openData(s)}>تعديل البيانات</button>:null}
            {permissions.reports?<button className="edit-report" onClick={()=>openReport(s)}>التقارير</button>:null}
            {permissions.finance?<button className="edit-finance" onClick={()=>openFinance(s)}>الحسابات</button>:null}
          </div></td>
        </tr>)}</tbody>
      </table>
