...
export default async function ParentApp({
  ...
  const billingProfiles=await dbSelect(
    "student_billing_profiles",
    `select=*&student_id=eq.${encodeURIComponent(student.id)}&limit=1`
  );
  const billingProfile=billingProfiles[0]||null;
  const ledger=billingProfile?await dbSelect(
    "financial_transactions",
    `select=*&student_id=eq.${encodeURIComponent(student.id)}&currency=eq.${encodeURIComponent(billingProfile.currency)}&order=transaction_date.desc,created_at.desc`
  ):[];
  ...
  return <div className="old-parent-app-v61">
    <div className="old-parent-phone-v61">
      ...
      {tab==="home" && <main className="home-page-v62">
        ...
        <section className="home-contact-v62">
      ...
      <nav className="old-bottom-nav-v61">
