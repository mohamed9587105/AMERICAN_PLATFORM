...
export function createSession(data:Record<string,any>,expiresMs=1000*60*60*24*30){
  const payload=Buffer.from(JSON.stringify({...data,exp:Date.now()+expiresMs})).toString("base64url");
