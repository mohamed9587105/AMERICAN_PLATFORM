...
export async function resolveCourse(input:string){
  ...
  const bySlug=await dbSelect("courses",`select=id,name,slug,exam_type&slug=eq.${encodeURIComponent(normalizedSlug)}&is_active=eq.true&limit=1`);
  if(bySlug[0]) return bySlug[0];

  // Smart matching: tolerate Beginner/Beginners, spaces, punctuation and similar labels.
  const all=await dbSelect("courses","select=id,name,slug,exam_type&is_active=eq.true&order=sort_order.asc");
  const wanted=normalizeCourseName(raw);
  const exactNormalized=all.find((c:any)=>normalizeCourseName(String(c.name||""))===wanted||normalizeCourseName(String(c.slug||"").replace(/-/g," "))===wanted);
  if(exactNormalized) return exactNormalized;

  const contains=all.filter((c:any)=>{
    const n=normalizeCourseName(String(c.name||""));
    return n.includes(wanted)||wanted.includes(n);
  });
  if(contains.length===1) return contains[0];

  const examAlias:Record<string,string>={
    "sat":"SAT","sat advance":"SAT","sat advanced":"SAT",
    "est":"EST","est advance":"EST","est advanced":"EST",
  };
  const examType=examAlias[wanted];
  if(examType){
    const rows=all.filter((c:any)=>String(c.exam_type||"").toUpperCase()===examType);
    if(rows[0]) return rows[0];
  }

  const available=all.map((c:any)=>c.name).filter(Boolean).join(" | ");
  throw new Error(`COURSE_NOT_FOUND:${raw}${available?` | AVAILABLE:${available}`:""}`);
}

export async function createStudentWithParents(input:NewStudentInput){
  if(!input.name?.trim()) throw new Error("STUDENT_NAME_REQUIRED");
  if(!input.course?.trim()) throw new Error("COURSE_REQUIRED");
  if(!input.parentPhone?.trim()) throw new Error("PARENT_PHONE_REQUIRED");
